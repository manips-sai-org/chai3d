...
int main(int argc, char *argv[])
{
    ...
    ODEMonkey = new cODEGenericBody(ODEWorld);
    ...
    ODEMonkey->setImageModel(object);

    ODEMonkey->createDynamicMesh(false);
...
void updateHaptics(void)
{
    ...
    // --- NEW: previous tool world position (initialize before loop) ---
    cTransform initWorld_T_tool = tool->getDeviceGlobalTransform();
    cVector3d prevToolGlobalPos = initWorld_T_tool.getLocalPos();

    // tuning parameter: how much of the tool displacement is transferred to object
    const double pushGain = 1.0; // try values <1.0 to reduce motion (e.g. 0.5)
    ...
    while (simulationRunning)
    {
        ...
        if (resetRequested.load())
        {
            ...
            state = INIT; // return to init state (your existing model)

            // stop forces applied to device
            ...
            // reinitialize the tool to clear proxy/contact states
            tool->initialize();
        ...
        if (state == IDLE)
        {
            ...
            cVector3d posDevice = tool->m_hapticPoint->getGlobalPosProxy();
            cMatrix3d rotDevice = tool->getDeviceGlobalRot();
            ...
            // get position
            cVector3d posMonkey = ODEMonkey->getLocalPos();
            ...
            // cVector3d springForce = -K_spring * posMonkey - D_damping * velMonkey;
            cVector3d springForce = -K_spring * posMonkey;

            // apply to ODE body
            ODEMonkey->addExternalForce(springForce);
